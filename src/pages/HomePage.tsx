          ...
          <AnimatePresence mode="wait" onExitComplete={() => setLayoutViewState(viewState)}>
                ...
                <div className="hero-text-container" style={{ width: '100%' }}>
                  <div className="hero-logo">
                    ...
                    <motion.div
                      ...
                      style={{ width: 80, height: 2, background: 'var(--ochre)', marginTop: 16, transformOrigin: 'left' }} />
                  </div>
                ...
                <div className="form-grid">
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                      {[3, 5, 7, 10, 14].map(d => (
                        <button key={d} type="button" onClick={() => setDays(d)} style={{ padding: '3px 9px', borderRadius: 6, border: `1px solid ${days === d ? 'var(--prussian)' : 'var(--color-border)'}`, background: days === d ? 'var(--prussian)' : 'transparent', color: days === d ? '#FFF' : 'var(--mist)', fontSize: 11, cursor: 'pointer', fontFamily: 'var(--font-serif)', transition: 'all 120ms' }}>{d}</button>
                      ))}
                    </div>
                  </div>
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                      {[30000, 50000, 80000, 100000].map(b => (
                        <button key={b} type="button" onClick={() => setBudget(b)} style={{ padding: '3px 9px', borderRadius: 6, border: `1px solid ${budget === b ? 'var(--prussian)' : 'var(--color-border)'}`, background: budget === b ? 'var(--prussian)' : 'transparent', color: budget === b ? '#FFF' : 'var(--mist)', fontSize: 11, cursor: 'pointer', fontFamily: 'var(--font-serif)', transition: 'all 120ms' }}>{b.toLocaleString()}</button>
                      ))}
                    </div>
                  </div>
                </div>
            ...
            {viewState === 'explore' && (
              ...
              <button
                ...
                style={{ background: 'transparent', border: 'none', color: 'var(--mist)', display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', marginBottom: 16, fontSize: 14, fontWeight: 500, fontFamily: 'var(--font-serif)', padding: '4px 8px 4px 0', transition: 'color 150ms', minHeight: 44 }}
                onMouseEnter={e => e.currentTarget.style.color = 'var(--prussian)'}
                onMouseLeave={e => e.currentTarget.style.color = 'var(--mist)'}
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M19 12H5M12 19l-7-7 7-7" /></svg>
                          ...
                          >
                            ...
                            <button
                              ...
                              style={{
                                ...
                                cursor: 'pointer', fontFamily: 'var(--font-serif)', transition: 'all 0.18s',
                              }}
                            >
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 10 }}>
                      ...
                      {collected.length > 0 && (
                        <motion.button
                          ...
                          style={{ padding: '10px 18px', border: '1px solid var(--vermillion)', borderRadius: 8, background: loading ? '#ccc' : 'var(--vermillion)', color: '#fff', fontSize: 13, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-serif)', opacity: loading ? 0.7 : 1 }}
                        >
                          {loading ? (
