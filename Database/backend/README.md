...
這個 Django 後端和 repo 裡的 `server/` FastAPI 共用同一個 repo root `.venv`。

如果你還沒建立虛擬環境，請在 repo root 執行：
...
```bash
./test_user_preferences.sh http://127.0.0.1:8000 1
```
