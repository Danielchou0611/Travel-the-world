...
logger = logging.getLogger(__name__)
# --- 1. RAG 檢索器 (Retriever) ---
def retrieve_local_knowledge(destination: str, min_rating: float = 3.0) -> str:
    """
    從新的 JSON 結構讀取景點資料。
    注意：使用 'region' 來對應 destination。
    """
    try:
        with open("japan_with_rating_interest.json", "r", encoding="utf-8") as f:
            db_data = json.load(f)
            
        # 1. 篩選：確保 region 相符，且評分達標
        filtered_places = [
            place for place in db_data 
            if destination in place.get("region", "") and place.get("google_rating", 0) >= min_rating
        ]
        
        # (選擇性優化) 2. 排序：直接讓 RAG 優先提供 xai_score 最高的景點給 AI
        filtered_places = sorted(filtered_places, key=lambda x: x.get("google_rating", 0), reverse=True)
        
        # 只取前 30 名，避免 Token 爆炸
        top_places = filtered_places[:30]
        
        if not top_places:
            return "目前資料庫無該地區的景點資料。"
            
        # 3. 轉為文字 Context：把強大的交通與分數特徵加進去
        context_lines = []
        for p in top_places:
            interests_str = ", ".join(p.get("interests", []))
            # 將交通距離與 xai_score 放進去讓 AI 參考
            line = (f"- {p['name']} ({p.get('google_name_matched', '')}) | "
                    f"評分: {p['google_rating']} ({p['review_count']}則) | "
                    f"標籤: {interests_str} | "
                    f"交通: 距 {p['station_anchor']} {p['distance_to_station_km']}km | "
                    )
        ...
        logger.error("找不到 attractions_db.json 檔案！")
        return ""
...
def build_rag_prompt(destination: str, user_prefs: dict, rag_context: str) -> str:
   ...
   👉 🚨【重要退路】：如果我提供的【官方景點候選清單】數量不夠填滿 {days} 天，請允許在「剩餘的天數或時段」中，自行推薦該地區真實存在的高評價景點來補足行程，但請在 xai_reason 註明「(AI 擴充推薦)」
    ...
    4. 4. "xai" 欄位規範：
       ...
       - 50字以內
    5. 🚨 【格式絕對限制】：請直接輸出 JSON 內容，絕對不要加上 ```json 的 Markdown 標記，也絕對不要在 JSON 前後加上任何問候語或額外文字！
    """
                ...
                "properties": {
                    ...
                    "attractions": {
                        ...
                        "items": {
                            ...
                            "required": ["id", "name", "category", "description", "duration", "estimatedCost", "location", "lat", "lng", "xai"]
                        }
                    }
                },
...
if __name__ == "__main__":
    ...
    if initial_result["status"] == "success":
        ...
        print("\n--- 初版行程 ---")
        for day in current_itinerary_state.get("itinerary", []):
            print(f"第 {day['day_number']} 天 | {day['place_name']} ({day['category']}) - 推薦理由: {day['xai_reason']}")
        ...
        if modify_result["status"] == "success":
            ...
            print("\n--- 修改後的行程 ---")
            for day in updated_itinerary_state.get("itinerary", []):
                print(f"第 {day['day_number']} 天 | {day['place_name']} ({day['category']}) - 推薦理由: {day['xai_reason']}")
