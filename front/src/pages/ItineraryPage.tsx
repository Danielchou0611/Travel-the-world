...
async function handleDownloadPdf() {
  ...
  try {
    ...
    const filename = `專屬行程規劃_${trip.preferences?.days ?? '未知'}天.pdf`;

    const response = await fetch('http://localhost:8001/api/export-pdf', {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorText || `PDF API failed: ${response.status}`);
    }
  ...
  try {
    ...
    const pdfTarget = (container.querySelector('.container') as HTMLElement) || container;
    ...
    const pdfHeightMm = (contentHeightPx * pdfWidthMm) / contentWidthPx;
    ...
    const opt = {
      ...
      jsPDF: {
        unit: 'mm',
        // 多補一點高度，避免最後一點內容被切到第 2 頁
        format: [pdfWidthMm, pdfHeightMm + 10],
        orientation: 'portrait',
      },
      // 長頁 PDF 不需要 html2pdf 自己幫你切頁
      pagebreak: {
        mode: [],
      },
    };

    // 5. 呼叫 html2pdf 進行轉換與下載
  ...
  /* async function handleDownloadPdf() {
    ...
    try {
      ...
      // 3. 設定 PDF 的輸出參數
      const opt = {
        margin:       15, // 上, 右, 下, 左 的邊距 (mm)
        filename:     `專屬行程規劃_${trip.preferences?.days ?? '未知'}天.pdf`,
        image:        { type: 'jpeg', quality: 0.98 }, // 圖片品質
        html2canvas:  { 
          scale: 2,           // 提高解析度讓文字不模糊
          useCORS: true       // 🚨 重要：允許載入跨域的景點圖片
        }, 
        jsPDF:        { unit: 'mm', format: [210, pdfHeightMm + 2], orientation: 'portrait' }
      };

      // 4. 呼叫 html2pdf 進行轉換與下載
