...
export default function PlanPPage() {
  ...
  // 🌟 新增：用來儲存 AI 排好的 ID 順序，以及 Loading 狀態
  ...
  // 🌟 核心修改：計算目前畫面上的景點，並套用 AI 排序
  
  // 🌟 核心修改：計算目前畫面上的景點，並套用 AI 排序
  const themes = useMemo(() => {
    const lockedPool = [...collected];                         
    const dynamicPool = shuffle(FALLBACK_POOL, shuffleSeed);          
    let all = [...lockedPool, ...dynamicPool].slice(0, 12);

    // 如果 AI 已經排好順序了，我們就根據 customOrder 的 ID 順序來重新排列 all 陣列
  ...
  }, [collected, shuffleSeed, customOrder]); // 依賴項加入 customOrder
  /* const themes = useMemo(
 ...
 */
  ...
  function reshuffle() {
    ...
    setCustomOrder(null); // 🌟 換一輪的時候，把 AI 的排序洗掉
  }

  // 🌟 新增：呼叫後端 AI 進行智能排序的函數
  async function handleAiSort() {
    setIsSorting(true);
    
    // 把目前畫面上的 12 個景點抽出來
    const currentSpots = themes.flatMap(t => t.spots);

    try {
      // 呼叫我們剛剛在 FastAPI 寫好的 Endpoint
      const response = await fetch("http://127.0.0.1:8001/api/sequence_p_route", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ spots: currentSpots }),
      });

      const result = await response.json();
      
      if (result.status === "success") {
        // 將 AI 回傳的 ["f3", "a01", "f5"...] 存入狀態
        // React 會自動觸發上面的 useMemo 重算，卡片就會帶有動畫地飛到新位置！
        setCustomOrder(result.data);
      } else {
        alert("AI 思考失敗了，請再試一次！");
      }
    } catch (error) {
      console.error("排序請求失敗:", error);
      alert("連線失敗，請確認後端已啟動。");
    } finally {
      setIsSorting(false);
    }
      ...
      >
        ...
        >
          ...
          <motion.button
            ...
            className="plan-p-action-btn font-mincho"
            style={{ 
              background: isSorting ? "var(--grey-light)" : "var(--primary-light)",
              borderColor: "var(--primary)" 
            }}
          >
            {isSorting ? "✨ 感受氣氛中..." : "✨ AI 順一下"}
          </motion.button>
