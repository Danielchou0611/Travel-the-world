...
- **Google Maps 評分爬取**：精準對應 Google 地圖上的星等評分與評論總數。
...
### 2. 處理腳本 (Python)
- **`scrape_ratings.py`**: **主程式**。負責呼叫 Scraper 工具抓取評分。支援 `--repair` 模式，可針對匹配不準確的項目重新抓取。
- **`japan_split.py`**: **數據源抓取器**。使用 SPARQL 查詢從 Wikidata API 抓取各縣市的原始景點資訊。
- **`merge_japan_data.py`**: 將 `japan_data/` 內的分散檔案合併為全日本總表。

### 3. 設定與說明
- **`.gitignore`**: 排除暫存檔與備份檔，保持 Git 倉庫乾淨。
- **`README.md`**: 使用手冊。

## 🛠 使用工具
- **程式語言**: Python 3.x
- **爬蟲引擎**: [google-maps-scraper](https://github.com/gosom/google-maps-scraper) (Go 編譯版)
