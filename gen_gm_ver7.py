...
def retrieve_local_knowledge(destination: str, required_count: int) -> str:
    """
    從 JSON 讀取景點。若符合的高分景點數量不足，會自動降低星等標準。
    """
    try:
        ...
        for min_rating in thresholds:
            ...
            # 排序：優先推 xai_score，如果沒有就用 google_rating
            ...
            if len(filtered_places) >= required_count:
                ...
                logger.info(f"🔍 檢索成功：使用最低星等 {min_rating}，找到 {len(top_places)} 個候選景點")
                break
            else:
                # 先把目前找到的存起來，萬一降到 0.0 都不夠，至少還有這些
                top_places = filtered_places[:30] 

        if not top_places:
            logger.warning(f"⚠️ 資料庫完全沒有 {destination} 的資料！")
        ...
        logger.error("找不到 japan_with_rating_interest.json 檔案！")
        return ""
...
def build_rag_prompt(destination: str, user_prefs: dict, rag_context: str) -> str:
   ...
   👉 🚨【重要退路】：如果我提供的【官方景點候選清單】數量不足以填滿 {days} 天的行程，請允許你在「剩餘的空白時段」中，自行推薦該地區真實存在、且符合使用者興趣的高評價景點來補足。
