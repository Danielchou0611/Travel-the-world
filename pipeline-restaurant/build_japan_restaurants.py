#!/usr/bin/env python3
"""Standalone Japan restaurant pipeline: JSON -> normalized CSV -> scored CSV."""
...
SOURCE_NAME = "japan_restaurant_with_rating"
...
def should_exclude_type(raw_type: str) -> bool:
    normalized = raw_type.strip()
    if normalized in EXCLUDED_TYPE_EXACT:
        return True
    if any(keyword in normalized for keyword in EXCLUDED_TYPE_CONTAINS):
        return True
    return False
...
def build_scored_rows(rows: list[dict[str, str]]) -> tuple[list[dict[str, str]], dict[str, int]]:
    if not rows:
        return [], {"scored_rows": 0, "skipped_rows": 0}

    review_values = [max(0, int(as_float(row["google_review_count"], DEFAULT_REVIEW_COUNT))) for row in rows]
    ...
    for row in rows:
        ...
        distance_to_station_km = ""
        station_distance_efficiency = ""
        if row["prefecture"] in STATION_ANCHORS:
            ...
            if row["lat"] and row["lng"]:
                ...
                distance = haversine_km(lat, lng, anchor_lat, anchor_lng)
                distance_to_station_km = f"{distance:.1f}"
                station_distance_efficiency = f"{1 / (1 + distance / 5):.4f}"

        lat = as_float(row["lat"], 0.0) if row["lat"] else None
        lng = as_float(row["lng"], 0.0) if row["lng"] else None
        scored_rows.append(
            {
                "id": row["source_id"],
                "name": row["name"],
                ...
                "station_distance_efficiency": station_distance_efficiency,
    ...
    return scored_rows, {"scored_rows": len(scored_rows), "skipped_rows": 0}
